...
private["_groupID","_uniqueID", "_radarStationPos", "_playerSide", "_markerState", "_enemyCount", "_friendlyCount", "_currSide", "_runLoop", "_stillAlive"];

_uniqueID = _this select 0;
_radarStationPos = _this select 1;
_playerSide = _this select 2;
_markerState = _this select 3;
_groupID = _this select 4;
    ...
    if(_enemyCount > 0 AND _friendlyCount > 0) then {
...

   	{
    	...
    	if(_x select 0 == _uniqueID) then {
			...
			clientRadarMarkers set [count clientRadarMarkers,[_uniqueID,_radarStationPos,_playerSide, _markerState, _groupID]];
