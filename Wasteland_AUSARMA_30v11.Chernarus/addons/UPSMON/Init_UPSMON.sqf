...
//Adding eventhandlers
	//"KRON_UPS_EAST_SURRENDED" addPublicVariableEventHandler { if (_this select 1) then { nul=[east] execvm "addons\UPSMON\UPSMON\MON_surrended.sqf";};};
	//"KRON_UPS_WEST_SURRENDED" addPublicVariableEventHandler { if (_this select 1) then { nul=[west] execvm "addons\UPSMON\UPSMON\MON_surrended.sqf";};};
	//"KRON_UPS_GUER_SURRENDED" addPublicVariableEventHandler { if (_this select 1) then { nul=[resistance] execvm "addons\UPSMON\UPSMON\MON_surrended.sqf";};};
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	if (isNil("KRON_UPS_INIT") || KRON_UPS_INIT == 0) then {
		...
		call compile preprocessFileLineNumbers "addons\UPSMON\UPSMON\common\MON_functions.sqf";
		
		//init !R functions
		call compile preprocessFileLineNumbers "addons\UPSMON\UPSMON\!R\R_functions.sqf";
		
		if (isNil "RE") then {[] execVM "\ca\Modules\MP\data\scripts\MPframework.sqf"}; 
		
		//scripts initialization
		UPSMON = compile preprocessFile "addons\UPSMON\UPSMON.sqf";	
		UPSMON_surrended = compile preprocessFile "addons\UPSMON\UPSMON\MON_surrended.sqf";	
		...
		MON_MAIN_server = {
			...
			while {true} do 
			{
                ...
                
				{
					...
					if (!isnull _npc_check && alive _npc_check && !captive _npc_check) then {
						...
						
						{
							...
							if (side _target in _enemyside) then {
								...
								if (!isnull _target && alive _target && canmove _target && !captive _target && ((_npc knowsabout _target) > R_knowsAboutEnemy) && (_target iskindof "Land" || _target iskindof "Air" || _target iskindof "Ship") && !(_target iskindof "Animal") && (((_target emptyPositions "Gunner") == 0) && ((_target emptyPositions "Driver") == 0) || (!isnull (gunner _target) && canmove (gunner _target)) || (!isnull (driver _target) && canmove (driver _target)))) then {
