...
//    nul=[this,"area0"] execVM "addons\UPSMON\upsmon.sqf";
//
//  Defensive squad samples:
//    nul=[this,"area0","nomove"] execVM "addons\UPSMON\upsmon.sqf";
//
//  Reinforcement
//    nul=[this,"area0","reinforcement:",1] execVM "addons\UPSMON\UPSMON.sqf";
...
// Postioning
private["_currcycle","_targetX","_targetY","_relTX","_relTY","_relUX","_relUY","_waiting","_pursue","_react","_newpos","_currPos","_orgPos","_targetPos","_attackPos","_flankPos","_avoidPos", "_speedmode"];
...
private["_objsflankPos1","_cntobjs1","_objsflankPos2","_cntobjs2","_targettext","_dir1","_dir2","_dir3","_dd","_timeontarget","_dirf1","_dirf2","_fightmode",
		...
		,"_track","_membertypes","_respawn","_respawnmax","_lead","_safemode","_vehicles","_dist3","_lastwptype","_template","_unittype","_initstr","_fortifyorig","_nowpType","_ambushtype" ,"_vehicletypes", "_onroad","_loop2", "_tries2","_targetPosTemp","_sokilled","_sowounded","_nosmoke","_newunit","_rlastpos","_rcurrpos","_jumpers","_isSoldier","_noveh","_deadBodiesReact"];
...
// unit that's moving
_obj = _this select 0; //group or leader
_npc = _obj;
_npc = leader (_obj);
...
_grpidx = format["%1%2",_grpid,IsServer];
...
if (_isplane) then {_closeenough = KRON_UPS_closeenough * 3};
...
// TBD: add to global side arrays?
	...
	// create the clones
		for "_grpcnt" from 1 to _copies do {
			...
			nul=[_lead,_areamarker,_pause,_noslow,_nomove,_nofollow,_initpos,_track,_showmarker,_shareinfo,"DELETE:",_dead] execVM "addons\UPSMON\upsmon.sqf";
			//sleep .05;
		};	
		processInitCommands;
		sleep .05;
...
while {_loop} do {
	...
	if ((simulationEnabled _npc) || (simulationEnabled (vehicle _npc))) then {
	...
	//	if (KRON_UPS_Debug>0) then {player sidechat format["%1 rea=%2 wai=%3 tim=%4 tg=%5 %6",_grpidx,_react,_waiting,_timeontarget,typeof _target,alive _target]};							
		...
		if (count _members == 0) then {
			...
			if (!alive _npc || !canmove _npc || isplayer _npc) then {
