...
//_position = _unit getVariable "spawnpos";
//if ((isNil _position select 0) or (isNil _position select 1) or (isNil _position select 2)) then { _position = getPosASL _unit; };
_type = typeOf _unit;
_dead = false;
_nodelay = false;
#ifdef __A2NET__
...
{	
	...
	if(_result == 1) then
    {
		...
		if (_deserted > 0) then
		{
			//if ((getPosASL _unit distance _position > 10) and ({alive _x} count crew _unit == 0) and (getDammage _unit < 0.8) and (isNull (_unit getVariable "R3F_LOG_remorque")) and (count (getPosASL _unit nearEntities [["Man"],50]) == 0)) then
			if ((getPosASL _unit distance _position > 10) and ({alive _x} count crew _unit == 0) and (getDammage _unit < 0.8) and (isNull (_unit getVariable "R3F_LOG_remorque"))) then
			{
				#ifdef __A2NET__
				...
				sleep 0.1;
				#ifdef __A2NET__
			 	//waitUntil {_timeout < netTime or !alive _unit or {alive _x} count crew _unit > 0 or !isNull (_unit getVariable "R3F_LOG_remorque") or count (getPosASL _unit nearEntities [["Man"],50]) > 0};
			 	waitUntil {_timeout < netTime or !alive _unit or {alive _x} count crew _unit > 0 or !isNull (_unit getVariable "R3F_LOG_remorque")};
				#else
				//waitUntil {_timeout < time or !alive _unit or {alive _x} count crew _unit > 0 or !isNull (_unit getVariable "R3F_LOG_remorque") or count (getPosASL _unit nearEntities [["Man"],50]) > 0};
				waitUntil {_timeout < time or !alive _unit or {alive _x} count crew _unit > 0 or !isNull (_unit getVariable "R3F_LOG_remorque")};
				#endif
				if ({alive _x} count crew _unit > 0) then {_dead = false};
				if ({alive _x} count crew _unit == 0) then {_dead = true; _nodelay =true};
                                if !(isNull (_unit getVariable "R3F_LOG_remorque")) then {_dead = false};
                                //if (count (getPosASL _unit nearEntities [["Man"],50]) > 0) then {_dead = false};
