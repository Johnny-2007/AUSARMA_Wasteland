...
aActionsIDs = aActionsIDs + [player addAction[("<t color=""#21DE31"">Beacon Detector</t>"), "client\actions\detectBeacon.sqf", 1, 1, false, false, "", 'player distance (nearestobjects [player, ["Satelit"],  300] select 0) < 300 AND ((nearestobjects [player, ["Satelit"],  300] select 0) getVariable "faction") != "WORLD"']];
...
aActionsIDs = aActionsIDs + [player addAction[("<t color=""#21DE31"">Right Overturned Vehicle</t>"), "client\actions\unflipVehicle.sqf", [], 1, false, false, "", 'player distance (nearestobjects [player, ["Car","Motorcycle","Tank"], 10] select 0) < 10 AND (count (crew (nearestobjects [player, ["Car","Motorcycle","Tank"], 10] select 0)))== 0 AND ((vectorUp (nearestobjects [player, ["Car","Motorcycle","Tank"], 10] select 0)) select 2) < 0.5 ']];
